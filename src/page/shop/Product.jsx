...
const Product = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (productsData.length > 0) {
          const maxPriceValue = Math.max(...productsData.map(p => Number(p.harga)));
    ...
    if (selectedCategory !== 'all') {
      filtered = filtered.filter(product =>
        ...
        ((product.category && product.category.toLowerCase() === selectedCategory.toLowerCase()) ||
         (product.nama_category && product.nama_category.toLowerCase() === selectedCategory.toLowerCase()))
      );
    }
  ...
  const categories = [
    ...
    ...Array.from(
      new Set(
        products
          .map(p => p.category || p.nama_category)
          .filter(Boolean)
      )
    )
  ];
        ...
        {loading ? (
          ...
          <div className={viewMode === 'grid' 
            ...
            {filteredProducts.map((item) => {
              ...
              if (viewMode === 'list') {
                return (
                  <div key={item.id_product} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex">
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-between">
                      <div>
                        <h3 className="text-xl font-bold text-gray-800 mb-2">{item.nama_produk}</h3>
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-gray-500">
                              {item.stok > 10 ? 'Tersedia' : item.stok > 0 ? `Sisa ${item.stok}` : 'Habis'}
                            </span>
                          </div>
                          {cartQuantity > 0 && (
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          disabled={item.stok === 0 || cartQuantity >= item.stok || addLoadingId === item.id_product}
                          className="flex-1 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-semibold px-6 py-2 rounded-lg transition-all duration-300 disabled:from-gray-400 disabled:to-gray-400 disabled:cursor-not-allowed"
                        >
                          {addLoadingId === item.id_product ? 'Menambah...' : 
                           item.stok === 0 ? 'Habis' :
                           cartQuantity >= item.stok ? 'Maksimal' : 'Tambah ke Keranjang'}
                        </button>
                        <Link 
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-gray-700 mb-2">Tidak Ada Produk</h3>
            <p className="text-gray-500 mb-4">Tidak ditemukan produk dengan filter yang dipilih</p>
            <button 
              onClick={() => {
